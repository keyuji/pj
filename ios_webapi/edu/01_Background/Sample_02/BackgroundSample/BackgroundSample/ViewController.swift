...
class ViewController: UIViewController {
    ...
    @IBAction func decide(sender: UIButton) {
        ...
        queue.addOperationWithBlock { 
            ...
            guard text.characters.count < 10,
                ...
                // 処理が完了したことを、ローカル通知でユーザへ通知
                self.notifyCompleteDecide()
    ...
    /// ローカル通知を作成し、通知を行う
    func notifyCompleteDecide() {
        let application = UIApplication.sharedApplication()
        
        // ローカル通知を作成
        let notification = UILocalNotification()
        notification.alertTitle = "完了"
        notification.alertBody = "処理が完了しました。"
        notification.alertAction = "アプリへ移動"
